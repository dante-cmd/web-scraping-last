...
link_botica = 'https://www.boticasysalud.com/tienda/ofertas'

driver_path = r'C:\dchrome\chromedriver.exe'
driver = webdriver.Chrome(executable_path =driver_path) #driver_path, chrome_options=options)
driver.get(link_botica)
driver.find_element_by_class_name('row').send_keys('\n')
...
for element in page.find_all('img'):
    #print(element)
    if element.get('src'):
        text_url = element.get('src')
        if re.search(r'^http.+',text_url):
            print(element.get('src'))#, type(element.get('src')))
            img_list.append(element.get('src'))

img_list
for i,img in enumerate(img_list):
    response = requests.get(img)
    print(response.status_code)
    with open(rf'C:\Users\LENOVO\Desktop\python_course\datascience\web_scraping\img_{i}.png', 'wb') as file_ol:
        file_ol.write(response.content)
